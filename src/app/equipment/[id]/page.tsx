...
export default function EquipmentDetailPage() {
    ...
    const handleBack = () => {
        router.push('/?tab=NoticeBoard&noticeTab=용차출동');
    };
        ...
        if (isMobile && navigator.share) {
            try {
                await navigator.share({
                    title: item.equipment_name || '용차 정보',
                    text: `${item.equipment_name || '용차 정보'} - 용카에서 확인해보세요.`,
                    url: url,
                });
            } catch (err) {
    ...
    if (!item) {
        return (
            <div className="min-h-screen bg-white flex flex-col items-center justify-center p-6">
                ...
                <p className="text-gray-500 font-bold text-lg mb-6">존재하지 않는 정보입니다.</p>
                <button
                    onClick={handleBack}
    ...
    const tags = [
        ...(item.SubCategories || []),
        ...(item.TopCategories ? [item.TopCategories] : []),
        ...
        <div className="min-h-screen bg-white pb-32">
            ...
            <div className="px-5 pt-8 space-y-8">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                    {item.equipment_name && renderInfoRow('장비명', item.equipment_name, <Truck className="w-5 h-5" />)}
                    {item.equipment_career && renderInfoRow('경력', item.equipment_career, <Award className="w-5 h-5" />)}
                    {item.equipment_businessLicense && renderInfoRow('영업용유무', item.equipment_businessLicense, <Briefcase className="w-5 h-5" />)}
                    {item.equipment_rentalRates && renderInfoRow('요금/단가', item.equipment_rentalRates, <CreditCard className="w-5 h-5" />)}
                    {(item.address || item.deliverAddress) && renderInfoRow('위치/지역', item.address || item.deliverAddress, <MapPin className="w-5 h-5" />)}
                    {item.subRegion && renderInfoRow('상세지역', item.subRegion, <MapPin className="w-5 h-5" />)}
                    {item.equipment_phoneNumber && renderInfoRow('연락처', item.equipment_phoneNumber, <Phone className="w-5 h-5" />)}
                </div>

                <div className="space-y-4">
                    <div className="flex items-center gap-2 text-gray-900">
                        <FileText className="w-5 h-5" />
                        <h3 className="font-black">상세 소개</h3>
                    </div>
                    <div className="p-6 bg-gray-50 rounded-3xl text-gray-700 text-[15px] leading-relaxed whitespace-pre-wrap font-medium border border-gray-100 shadow-inner">
                        {item.equipment_career || '소개 내용이 없습니다.'}
                    </div>
                </div>
                ...
                <div className="p-5 bg-blue-50 rounded-2xl border border-blue-100 flex gap-3">
                    ...
                    <div className="flex-1">
                        <p className="text-xs font-black text-blue-800 mb-1">안내사항</p>
                        <p className="text-[11px] font-bold text-blue-600 leading-normal">
                            용차 이용 시 요금 등 상세 사항은 차주님과 직접 협의해 주시기 바랍니다.
                        </p>
                    </div>
                </div>
            </div>
            ...
            <div className="fixed bottom-0 left-0 right-0 p-5 bg-white/80 backdrop-blur-xl border-t border-gray-100 z-50 flex gap-3">
                <a
                    href={`tel:${item.equipment_phoneNumber}`}
                    className="flex-1 flex items-center justify-center gap-3 bg-blue-600 text-white rounded-2xl py-4 font-black text-base shadow-lg shadow-blue-200 hover:bg-blue-700 transition-all active:scale-95"
                >
                    <Phone className="w-6 h-6" />
                    전화 문의하기
                </a>
                <a
                    href={`sms:${item.equipment_phoneNumber}?body=${encodeURIComponent('안녕하세요. 용카에서 보고 연락드립니다.')}`}
            ...
            {/* Toast Notification */}
            {showToast && (
                <div className="fixed bottom-24 left-1/2 -translate-x-1/2 z-[300] animate-in slide-in-from-bottom-5 fade-in duration-300">
                    <div className="px-6 py-3 bg-gray-900/90 backdrop-blur-md text-white rounded-full font-bold text-sm shadow-2xl flex items-center gap-2">
